...
const Hero = ({ title, desc, buttonText, buttonTo }: HeroProps) => {
  return (
    <section className='mx-auto max-w-4xl py-2 sm:py-6 text-white '>
      <div className={cn('w-full bg-neutral-dark rounded-xl aspect-16/10 sm:aspect-16/8 shadow-2xl',
        ...
        {buttonText &&
          <button className='px-6 py-2 bg-primary hover:bg-secondary transition-all cursor-pointer w-fit rounded-lg text-base xs:text-lg font-semibold'>
            {buttonTo ?
              <NavLink to={buttonTo}>
                {buttonText}
              </NavLink>
